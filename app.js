  ...
  manuals: [
    {
      ...
      title: "受付（点検票・引換証）",
      summary:
        "受付は〜15:00まで、引取は〜16:00まで。1日最大60台（1日目は点検表No.60まで）。点検票の必須欄と引換証が最重要です。",
      steps: [
      ...
      fieldList: [
        ...
        { label: "氏名・連絡先", value: "いつでもつながる番号を記入してもらう。" },
        {
          label: "学生生活110番会員",
          value:
            "会員かどうかは近くのJBR担当に確認できる案内ができる。聞かれたらそのように案内する。会員に〇がついていれば会員証の提示を求める。会員であれば修理費が安くなることは、必要に応じて伝える。",
        },
        {
          label: "引取予定時刻",
          value: "引取は16:00までであるため、それ以降を指定されないように案内する。",
        },
        {
          label: "修理希望確認",
          value:
      ...
      detail: [
        ...
        "マニュアル本文では「受付は〜15:00まで」「引取は〜16:00まで」とあります。一方、台詞例では引き取りを「4限の終わりまで」と案内しています。掲示や運営指示と食い違う場合は、現地の指示を優先してください。",
        "点検票はGIが常に持ち、紛失しないよう管理します。記入後は引換証（小さな紙）を渡し、点検表と同じ番号と受取16:00までを明記し、なくさないよう伝えます。",
      ...
      dialogue: {
        ...
        lines: [
          { role: "GI", text: "こんにちは。自転車点検をご希望ですか？" },
          { role: "お客様", text: "はい。" },
          { role: "GI", text: "こちらの受付用紙にご記入をお願いします。" },
          { role: "GI", text: "学部学科、氏名、連絡先をはじめに記入してください。" },
          { role: "GI", text: "学生生活110番の会員ですか？" },
          { role: "お客様", text: "はい／いいえ。" },
          { role: "GI", text: "（会員の場合）会員証を見せてください" },
          {
            role: "GI",
            text: "引き取り時間の注意点ですが、最終受け取り時刻が4限の終わりまでとなっています。それを考慮してご記入ください。",
          },
          { role: "GI", text: "今回、無償点検と有償修理がありますが、どちらにしますか？" },
          { role: "お客様", text: "有償／無償でお願いします。" },
          {
            role: "GI",
            text: "（有償の場合）現金のみの対応です。修理費の上限をおいくらにしますか？",
          },
          { role: "お客様", text: "（上限）〇〇円です。" },
          {
            role: "GI",
            text: "気になる箇所の項目をご記入ください。詳しい症状やその他気になる箇所があれば、そちらにもご記入ください。",
          },
        ],
      },
    },
    ...
    },
    {
      ...
      detail: [
        "【無償点検のお客様】引換証を受け取ったうえで、対応する番号の点検票がある自転車を引き渡します。その際、点検票を見て点検内容をお伝えします。引換証の受け取りを忘れないでください。最後にチラシを渡して完了です。可能ならその場でアンケートフォームを読み込んでもらい、回答を促します（強制はしません）。",
        "【有償修理・お金関連は超重要】基本の流れは同じです。引換証を受け取ってから、対応番号の点検票の自転車を持ってきます。点検票を見て内容を伝えます。引換証の受け取り忘れに注意してください。",
        "受け渡す前に、点検票の一番下に書いてある金額を受け取ります。二役はお金のやり取りをしっかり監督します。特に30分ごとに金銭の矛盾がないか確認し、タイムスタンプを入れておきます。お釣りがある場合は電卓で計算し、お釣りを渡します。",
        "お金のやり取りが完了したら自転車を引き渡し、最後にチラシを渡して完了です。アンケートについても無償と同様です。",
      ],
    },
    {
      id: "tidy",
    ...
    },
    {
      ...
      detail: [
        "宣伝担当はその他全員とされています。開催場所の付近で宣伝を行い、興味がありそうな方にはどのようなことができるか説明します。",
    ...
    },
    {
      ...
      title: "役割分担と人数",
      summary:
        "受付4（うち1人は二役）、保管2、修理アシスト2。誘導・声かけは誘導フェイズを参照。",
      steps: [
        "受付4人（1人は二役）…受付・引取では自転車の移動以外",
        "保管2人…受付・整理・引取で自転車の移動",
        "修理アシスト2人…修理フェイズを担当",
        "誘導・宣伝の声かけは誘導フェイズの担当（その他全員）",
      ],
      detail: [
        "受付担当：4人（うち1人は二役であること）。受付フェイズ・引取フェイズでは、自転車の移動以外を行います。",
        "保管担当：2人。受付フェイズでは自転車の移動、整理フェイズでは一連の仕事、引取フェイズでは自転車の移動を行います。",
        "修理アシスト：2人。修理フェイズの一連の仕事を行います。",
